% crosstube_SDW(200,2);
crosstube_SDW_known_bubbles(200,1);
crosstube_SDW_known_bubbles(200,3);
crosstube_SDW_known_bubbles(200,4);
...
function crosstube_SDW(frames, apertures) 
    ...
    %% to beamform in gpu
    save(['../RF/rf_', filename,'dt005'], 'rf_data', '-v7.3');
    save(['../RF/rf_', filename,'bubbles_dt005'], 'tot_bub','-v7.3');
    %%
    % perform SVD
    ...
    for i = 1:20:UserSet.totalFrame-19
        ...
        saveIQ_simple(tmp, ['../BF/bf_', filename,num2str(i),'_',num2str(i+19),'_dt005'], pixelMap, UserSet)
    end
    %%
...
function crosstube_SDW_known_bubbles(frames, apertures) 
    ...
    load('../RF/rf_SDW_200F2A_Xbubbles.mat');
...
%     save(['../RF/rf_', filename,'bubbles'], 'tot_bub','-v7.3');
    ...
    %% Beamforming
    for i = 1:10:UserSet.totalFrame-9
        % by 10-frame chunks
        [tmp, pixelMap] = beamform_sim_ple(rf_data(:,:,i:i+9), transducer, [0,0], -32*lambda);
        tmp = sum(tmp,5);
        tmp = permute(tmp, [3,2,1,4]);
        saveIQ_simple(tmp, ['../BF/bf_', filename,num2str(i),'_',num2str(i+9)], pixelMap, UserSet)
    end
    %%
